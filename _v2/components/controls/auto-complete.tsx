...
import { ControlValueAccessorProps } from "../common/control-value-accessor-props";
import { useKeyPress } from "@/_v2/lib/use-key-press";
import { CreateControl } from "../common/create-control";
...
export interface AutocompleteProps<T>
    extends ControlValueAccessorProps<AutocompleteOption<T>[]> {
...
export const Autocomplete = forwardRef(function Autocomplete<T = any>(
    ...
    const fetchOptions = async (search: string) => {
        const resolvedOptions =
            (optionsResolver && (await optionsResolver(search))) ?? [];
        setOptions(
    ...
    const displayOptions = useMemo(() => {
        const selectedSet = new Set(
            value?.filter(Boolean)?.map((s) => s.label)
        );
        const omitSet = new Set(
            (omitOptions ?? []).filter(Boolean).map((s) => s.label)
        );
    ...
    useEffect(() => {
        if (displayOptions.length && downPress) {
            setCursor((prevState) =>
                prevState < displayOptions.length - 1
                    ? prevState + 1
                    : prevState
            );
        }
            ...
            <Popover.Anchor asChild>
                <Popover.Trigger className="w-full flex flex-1">
                    <div
                        ...
                        className="flex w-full rounded-md bg-background border border-border p-1 focus-within:outline outline-offset-2 outline-2 outline-border"
                    >
