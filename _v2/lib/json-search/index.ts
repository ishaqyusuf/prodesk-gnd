...
import tokenizeQuery from "./query-tokenizer";
...
function filterQueries(query, searchKeys, searchKeysToObjectKeyMap) {
    ...
    const containsTextRegex = tokens
        .filter((token) => token.key === undefined && !token.isNegate)
        .map((token) => RegExpEscape(token.q))
        .join("|");
    const notcontainsTextRegex = tokens
        .filter((token) => token.key === undefined && token.isNegate)
        .map((token) => RegExpEscape(token.q))
        .join("|");
    const containsKeyRegexs = tokens
        .filter((token) => token.key !== undefined && !token.isNegate)
        .map((token) => {
            return {
    ...
    const notContainsKeyRegexs = tokens
        .filter((token) => token.key !== undefined && token.isNegate)
        .map((token) => {
            return {
                key: searchKeysToObjectKeyMap[token.key],
    ...
    return {
        textSearch: containsTextRegex
            ? new RegExp(containsTextRegex, "ig")
            : undefined,
        textSearchNegate: notcontainsTextRegex
            ? new RegExp(notcontainsTextRegex, "i")
            : undefined,
...
export default class JsonSearch {
    ...
    constructor(jsonArray, options: any = {}) {
    ...
    filterFunc(query) {
        ...
        return (item) => {
            let itemMatched: any = true;
            let keyFound;

            let negatedKeywordNotFound: any = true;
            ...
            for (const [, objectKey] of Object.entries(this.indice)) {
                const value = item[objectKey as any];

                if (queryFuncs.textSearchNegate) {
                    negatedKeywordNotFound =
                        negatedKeywordNotFound &&
                        !testF(value, queryFuncs.textSearchNegate) &&
                        ++score;
                }
