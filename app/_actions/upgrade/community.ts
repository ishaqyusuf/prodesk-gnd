...
import { dotArray, removeEmptyValues, toDotNotation } from "@/lib/utils";
import { HomeTemplateMeta } from "@/types/community";
import { InstallCostMeta } from "@/types/settings";
  ...
  await Promise.all(
    templates.map(async (t) => {
      const _c = costs.filter(
        (c) => t.modelName?.toLocaleLowerCase() == c.model?.toLocaleLowerCase()
      );
      // .map((c) => c.id);
      if (_c?.length > 0) {
        ...
        if (grob.id) {
          grob.meta.totalCost = totalkValues(grob.meta?.costs);
          grob.meta.totalTax = totalkValues(grob.meta?.tax);
...
function _transformDesign(object) {
  ...
  Object.entries(object).map(([k, v]) => {
    const [k1, k2] = k.split(".").map(camelCaseKey);
    if (k1 && k2) {
      if (!tr[k1]) tr[k1] = {};
      tr[k1][k2] = v;
    }
