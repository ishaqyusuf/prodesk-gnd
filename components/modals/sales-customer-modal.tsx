    ...
    >
            ...
            <div key={i} className="space-y-2">
              ...
              <div className="  space-y-2 text-sm">
                {["name", "phoneNo", "address"].map((k, i) => {
                  const Icon = i == 2 ? MapPin : i == 1 ? Phone : User;
                  return (
                    <div
                      key={i}
                      className={`flex  space-x-2 ${i == 0 && "uppercase"}`}
                    >
                      <div>
                        <Icon className="h-4 w-4" />
                      </div>
                      <span
                        className={`leading-tight ${i == 2 && "line-clamp-2"}`}
                      >
                        {v[k] ?? "-"}
                      </span>
                    </div>
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <Tabs defaultValue={tab} className="">
          ...
          {(["billingAddress", "shippingAddress"] as AddressType[]).map(
            (t, i) => (
        ...
        <DialogFooter>
          <div className="flex w-full justify-between space-x-4">
            ...
            <Btn isLoading={loader.isLoading} onClick={submit} type="submit">
              Save changes
            </Btn>
          </div>
        </DialogFooter>
        ...
        <Select
          ...
          onValueChange={(value) => {
            ...
            if (selection) {
              ...
              form.setValue("profile.coefficient", selection.coefficient);
