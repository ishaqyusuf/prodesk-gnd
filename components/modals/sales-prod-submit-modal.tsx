      ...
      Content={({ data: ctx }) => (
        <>
          <div className="grid gap-4">
            ...
            <Info label="Status">
              <span>
                {ctx?.item.meta.produced_qty || 0} of {ctx?.item.qty} Completed
              </span>
            </Info>
            <Info
              label={
                ctx?.action == "Cancel" ? "Cancel Production Qty" : "Submit Qty"
              }
            >
              <div className="flex flex-wrap">
                {wPendingQty > 15 ? (
                  <Select
                    value={watchQty}
                    onValueChange={(e) => form.setValue("qty", e)}
                  >
                  ...
                  Array(wPendingQty)
                    ...
                    .map((_, i) => (
                      <Button
                        ...
                        className="m-1 w-10"
                        variant={watchQty == i + 1 ? "default" : "secondary"}
                        key={i}
                      >
                        {i + 1}
                      </Button>
