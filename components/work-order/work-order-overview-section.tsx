      ...
      <CardContent className="divide-y space-y-4 flex flex-col">
        <div className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-3">
          <Info label="Appointment">
            <DateCellContent>{workOrder.scheduleDate}</DateCellContent>{" "}
            <span>{workOrder.scheduleTime}</span>
          </Info>

          <Info label="Request Date">
            {<DateCellContent>{workOrder.requestDate}</DateCellContent>}
          </Info>
          <Info className="col-span-2 sm:col-span-3" label="Job Description">
